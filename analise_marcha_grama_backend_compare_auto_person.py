...
def process_video_mmpose(video_path, start_sec=None, end_sec=None, show_window=False,
                         model_alias="wholebody", device="cuda:0", use_whole_image=True, roi=None):
    """
    ...
    print(f"[INFO] Inicializando MMPose: pose2d={model_alias}, device={device}, det_model={'whole_image' if use_whole_image else 'default'}, roi={roi}")
    try:
    ...
    while cap.isOpened() and fidx <= f_end:
        ...
        # Se ROI foi definida, o modelo roda apenas no recorte, mas as coordenadas
        # são convertidas de volta para o frame original.
        if roi is not None:
        ...
        try:
            ...
            inst = _best_mmpose_instance(result.get("predictions"), prev_center=prev_center)
        except Exception as e:
        ...
        if inst is not None:
            ...
            # Rejeita poses anatômicas muito ruins em vez de salvar lixo do fundo.
            if _row_major_joint_score(row) < 0.08:
                fidx += 1
                continue
            ...
            if np.any(finite):
                prev_center = (float(np.nanmean(np.asarray(xs)[finite] - offset_x)),
                               float(np.nanmean(np.asarray(ys)[finite] - offset_y)))

            data.append(row)
            frames.append(frame.copy())

            if show_window:
                # Visual: destaca a ROI/pessoa e deixa o restante fosco.
                title = f"MMPose processando | frame {fidx} | Q para parar visualizacao"
                disp = draw_focus_overlay(frame, roi=roi, landmarks_row=row, title=title, dim_alpha=0.58)
                cv2.imshow("Processando MMPose - pessoa em foco", disp)
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    break
        fidx += 1
...
def process_video(video_path, start_sec=None, end_sec=None, show_window=False,
                  ...
                  mmpose_whole_image=True, mmpose_roi=None):
...
def ask_parameters(root, video_path, df_excel):
    ...
    tk.Label(frm, text="MMPose ROI mode:", anchor="w").grid(
        row=12, column=0, sticky="w", padx=8, pady=3)
    mmpose_roi_mode_var = tk.StringVar(value="auto_person")
    ttk.Combobox(frm, textvariable=mmpose_roi_mode_var,
                 values=["auto_person", "manual_roi", "full_frame"], state="readonly", width=16).grid(
        row=12, column=1, sticky="w", padx=8, pady=3)

    mmpose_whole_image_var = tk.BooleanVar(value=True)
    tk.Checkbutton(frm, text="MMPose: usar crop/ROI como pessoa única",
                   variable=mmpose_whole_image_var).grid(row=13, column=0, columnspan=2, pady=2)

    show_var = tk.BooleanVar(value=False)
    tk.Checkbutton(frm, text="Mostrar vídeo durante processamento",
                   variable=show_var).grid(row=14, column=0, columnspan=2, pady=4)
    ...
    tk.Button(frm, text="Continuar →", bg="#2196F3", fg="white",
              ...
              ).grid(row=15, column=0, columnspan=2, pady=8)
...
def run_analysis(root, video_path, output_path, df_excel):
    ...
    mmpose_roi_mode = params.get("mmpose_roi_mode", "auto_person")
    ...
    if backend == "mmpose":
        if mmpose_roi_mode == "auto_person":
            messagebox.showinfo(
                "MMPose Auto-Person",
